...
const Navbar = ({ user, setUser, darkMode, toggleDarkMode }) => {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0 z-[100] transition-all duration-300 ${scrolled ? "glass-nav py-3" : "bg-transparent py-6"}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              <Link to="/dashboard" className="text-sm font-semibold hover:text-blue-600 transition-colors">
                {user.name.split(' ')[0]}
              </Link>
              <IconButton icon={LogOut} onClick={() => { setUser(null); navigate("/"); }} />
            </div>
          ) : (
            <Link 
              to="/login"
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="max-w-4xl mb-16 space-y-6">
          ...
          <p className="text-xl text-slate-600 dark:text-slate-400 font-medium max-w-2xl leading-relaxed">
            The infrastructure connecting local talent with industrial scale opportunities. Streamlined, verified, and built for the modern workforce.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          ...
          <div className="bento-card col-span-1 md:col-span-2 lg:col-span-2 aspect-[4/3] md:aspect-auto md:h-[400px] group">
            ...
            <div className="absolute inset-0 z-10 p-8 flex flex-col justify-between">
              ...
              <div>
                <h3 className="text-3xl font-black text-white mb-2">Centralized Talent Hub</h3>
                <p className="text-slate-400 font-medium max-w-sm">Direct integration with 24 barangay registries and 120+ local enterprises.</p>
              </div>
            </div>
          </div>
          ...
          <div className="bento-card col-span-1 p-8 flex flex-col justify-between !bg-blue-600 border-none shadow-xl shadow-blue-900/20">
            <div className="space-y-4">
              ...
              <h3 className="text-3xl font-black text-white tracking-tight leading-none">Deploy Your <br /> Career.</h3>
              <p className="text-blue-50/80 text-sm font-medium leading-relaxed">Access the live database of local vacancies and government programs.</p>
            </div>
            <button 
          ...
          <div className="bento-card col-span-1 p-8 flex flex-col justify-between hover:border-blue-600/30">
            <div>
              ...
              <h4 className="industrial-label text-slate-400">Placement Rate</h4>
            </div>
            <div className="py-6">
               <div className="flex items-baseline gap-1">
                 <span className="text-6xl font-black text-slate-950 dark:text-white tracking-tighter">94</span>
                 <span className="text-2xl font-bold text-blue-600">%</span>
               </div>
               <p className="text-xs font-bold text-slate-500 mt-2 leading-relaxed uppercase tracking-wider">Verified deployments <br /> Q3 2024 fiscal year.</p>
            </div>
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
              <span className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em]">↑ 2.4% from Q2</span>
            </div>
          </div>
          ...
          <div className="bento-card col-span-1 md:col-span-2 p-8 flex flex-col justify-between">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={stat.title} className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 group hover:border-blue-600/20 transition-colors">
                  <p className={`text-3xl font-black ${stat.color} tracking-tighter`}>{stat.count}</p>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.15em] mt-2 leading-tight">{stat.title}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bento-card col-span-1 md:col-span-4 lg:col-span-2 p-8 flex items-center overflow-hidden bg-slate-50/50 dark:bg-slate-900/20">
             <div className="flex items-center gap-16 whitespace-nowrap animate-ticker font-black text-xl tracking-tighter text-slate-300 dark:text-slate-700">
               ...
               <span className="w-1.5 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full" />
               <span className="hover:text-blue-600/50 transition-colors">STA MARIA DEV</span>
               <span className="w-1.5 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full" />
               <span className="hover:text-blue-600/50 transition-colors">GLOBAL LOGISTICS</span>
               <span className="w-1.5 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full" />
               <span className="hover:text-blue-600/50 transition-colors">WALTERMART</span>
             </div>
          </div>
...
const Browse = () => {
  ...
  const filtered = useMemo(() => {
    return JOBS_DATA.filter(j => 
      (j.title.toLowerCase().includes(term.toLowerCase()) || j.company.toLowerCase().includes(term.toLowerCase())) &&
    ...
    <div className="min-h-screen pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
          <div>
            <Label icon={Search} className="mb-4">Database Query</Label>
            <h1 className="industrial-heading text-4xl text-slate-950 dark:text-white">Active Positions.</h1>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="border-t border-slate-200 dark:border-slate-800">
              {filtered.map(job => (
                <div key={job.id} className="group py-6 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors px-4 -mx-4 rounded-lg">
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-bold text-slate-950 dark:text-white group-hover:text-blue-600 transition-colors">{job.title}</h3>
                  ...
                  <div className="flex items-center gap-6 w-full md:w-auto">
                    <div className="text-right hidden sm:block">
                      <p className="industrial-label text-slate-400 mb-1">Compensation</p>
              ...
              {filtered.length === 0 && (
                 <div className="py-20 text-center text-slate-500">
                   ...
                   <p className="font-medium">No records found matching query parameters.</p>
                 </div>
              )}
            </div>
          </div>
    ...
    <div className="min-h-screen flex items-center justify-center bg-grid-pattern p-6">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="mb-8">
          ...
          <h2 className="industrial-heading text-3xl text-slate-950 dark:text-white mb-2">Authentication.</h2>
          <p className="text-slate-500 text-sm font-medium">Access secured municipal infrastructure.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1.5">
            <label className="industrial-label text-slate-500 block">Identifier (Email)</label>
            <input 
              type="email" 
              required
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              <label className="industrial-label text-slate-500 block">Access Key</label>
              <span className="industrial-label text-blue-600 cursor-pointer hover:underline">Reset</span>
            </div>
            <input 
              type="password" 
              required
          ...
          <button type="submit" className="w-full bg-slate-950 dark:bg-white text-white dark:text-slate-950 font-bold py-3 rounded-lg mt-4 hover:bg-blue-600 hover:text-white transition-colors">
    ...
    <div className="min-h-screen pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-12 border-b border-slate-200 dark:border-slate-800 pb-8">
          <div>
            <Label icon={ShieldCheck} className="mb-2">Verified Identity</Label>
            <h1 className="industrial-heading text-4xl text-slate-950 dark:text-white">{user?.name}</h1>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="bento-card p-6">
            ...
            <p className="text-2xl font-black text-slate-950 dark:text-white mt-2">Active</p>
            <p className="text-sm text-emerald-600 font-medium mt-1">Ready for placement.</p>
          </div>
          <div className="bento-card p-6">
            <Label>Submitted Proposals</Label>
            <p className="text-2xl font-black text-slate-950 dark:text-white mt-2">3</p>
          ...
          <div className="bento-card p-6 bg-slate-950 dark:bg-white text-white dark:text-slate-950">
            <Label className="!text-slate-400 dark:!text-slate-500">System Notice</Label>
            <p className="font-bold mt-2">TUPAD slot allocation begins next week.</p>
    ...
    <div className="bg-grid-pattern min-h-screen pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-12">
          ...
          <p className="text-slate-500 font-medium mt-4 max-w-2xl">Query live opportunities directly from the municipal database. Filter by strategic economic sector.</p>
        </div>
          ...
          <aside className="w-full lg:w-80 shrink-0 space-y-4">
            {sectors.map(sec => (
              ...
              >
                ...
                <span className={`industrial-label ${activeSector === sec.id ? "text-white/70 dark:text-slate-500" : ""}`}>{sec.count}</span>
              </button>
            ))}
          </aside>
          ...
          <div className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 md:p-10 shadow-sm relative overflow-hidden">
             ...
             <div className="space-y-4 relative z-10">
                {filteredJobs.map(job => (
                  <div key={job.id} className="group p-6 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-blue-600/30 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all flex flex-col md:flex-row gap-6 justify-between items-start md:items-center cursor-pointer">
                     <div>
                        ...
                        <h4 className="text-xl font-bold text-slate-950 dark:text-white group-hover:text-blue-600 transition-colors">{job.title}</h4>
  ...
  <div className="bg-grid-pattern min-h-screen pt-32 pb-24">
    <div className="max-w-7xl mx-auto px-6">
      <div className="max-w-4xl mb-16 space-y-6">
        ...
        <p className="text-xl text-slate-600 dark:text-slate-400 font-medium max-w-2xl leading-relaxed">
          Scale your local workforce with precision. Access the verified registry of Sta. Maria's talent and streamline your acquisition pipeline.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="bento-card p-8 group">
            ...
            <h3 className="text-2xl font-black text-slate-950 dark:text-white mb-4 tracking-tight">{item.title}</h3>
        ...
        <div className="bento-card col-span-1 md:col-span-2 p-10 bg-slate-950 text-white flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="space-y-4">
             <h3 className="text-3xl font-black tracking-tight">Accreditation Process</h3>
             <p className="text-slate-400 max-w-md font-medium">Is your company registered in Sta. Maria? Initialize your partner accreditation to start posting vacancies.</p>
          </div>
  ...
  <div className="bg-grid-pattern min-h-screen pt-32 pb-24">
    <div className="max-w-7xl mx-auto px-6">
      <div className="max-w-4xl mb-16 space-y-6">
        ...
        <p className="text-xl text-slate-600 dark:text-slate-400 font-medium max-w-2xl leading-relaxed">
          A centralized, high-performance ecosystem designed to streamline the economic integration of Sta. Maria, Bulacan.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bento-card col-span-1 md:col-span-2 p-10 border-blue-600/20">
           <div className="flex items-start gap-8">
              ...
              <div className="space-y-4">
                 ...
                 <h3 className="text-3xl font-black text-slate-950 dark:text-white tracking-tight">Barangay Data Link</h3>
                 <p className="text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
                   JobLinked connects directly with local barangay registries, ensuring every applicant is a verified resident of Sta. Maria. This protocol eliminates data redundancy and enhances screening security.
                 </p>
              </div>
           </div>
        </div>
        ...
        <div className="bento-card p-10 flex flex-col justify-between">
           <div className="space-y-6">
             ...
             <h3 className="text-2xl font-black text-slate-950 dark:text-white tracking-tight">Data Sovereignty</h3>
             <p className="text-sm text-slate-500 font-medium leading-relaxed">
               All employment data is hosted on secured local infrastructure, compliant with national data privacy standards.
             </p>
           </div>
        </div>
        ...
        <div className="bento-card col-span-1 md:col-span-3 p-12 bg-white dark:bg-slate-900 text-center space-y-8">
           ...
           <h3 className="text-4xl md:text-5xl font-black text-slate-950 dark:text-white tracking-tighter">Unified Workforce Monitoring.</h3>
      ...
      <footer className="border-t border-slate-200 dark:border-slate-800 py-12 bg-white dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="industrial-heading tracking-tight text-slate-500">JOBLINKED.</span>
          </div>
